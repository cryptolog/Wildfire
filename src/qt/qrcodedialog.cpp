...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("WFR")
    updateDisplayUnit();
}
...
{
    QString ret = QString("wildfire:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WFR unit input in lnReqAmount, we generate the URI with WFR as unit (as defined in BIP21)
