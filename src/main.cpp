...
#if defined(NDEBUG)
# error "Wildfire cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x26ed99591e005cddba3fbebbd9c63b22fe23b69a3b38cf89e76b93d9a6d55610");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wildfire: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wildfire Signed Message:\n";
...
{
    // Wildfire: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 2250005000 * COIN; //2.25 billion premine + 5000 default
    }
    else if(nHeight < 100000)  
    {
        nSubsidy = 12500 * COIN; //7500 for early mining + 5000 default for 100000 blocks = 1.25 billion
    }
    // Subsidy is cut in half every 4200000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 4200000); // Wildfire: 4200000 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Wildfire: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Wildfire: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wildfire: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wildfire: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x26ed99591e005cddba3fbebbd9c63b22fe23b69a3b38cf89e76b93d9a6d55610");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Washington Post 15/Jan/2018 Earthquake in Peru destroys dozens of homes, kills 1 man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515982514;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1291925;
        ...
        assert(block.hashMerkleRoot == uint256("0x4a3193c40a5fba9bb11dac4ddfb2f44d9967be2775df7e3353e07d2ee64f01dd"));
...
unsigned char pchMessageStart[4] = { 0xc2, 0xd3, 0xd2, 0xbe }; // Wildfire: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WildfireMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WildfireMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WildfireMiner(CWallet *pwallet)
{
    printf("WildfireMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wildfire-miner");
    ...
    try { loop {
        ...
        printf("Running WildfireMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WildfireMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WildfireMiner, pwallet));
}
